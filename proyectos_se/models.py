...
class Proyecto_SE_Estimado_Conceptual(models.Model):
    proyecto_se = models.ForeignKey(Proyecto_SE, on_delete=models.CASCADE, related_name="proyectos_SE_estimado_conceptual_se")
    estimado_conceptual = models.ForeignKey(Proyecto_E, on_delete=models.CASCADE, related_name="proyectos_SE_estimado_conceptual_e")

    def __str__(self):
        return f"Proyecto SE: {self.proyecto_se.codigo} - Estimado Conceptual: {self.estimado_conceptual.codigo}"
    

class Proyecto_SE_Licitacion(models.Model):
    proyecto_se = models.ForeignKey(Proyecto_SE, on_delete=models.CASCADE)
    licitacion = models.ForeignKey(Licitacion, on_delete=models.CASCADE)

    def __str__(self):
        return f"Proyecto SE: {self.proyecto_se.codigo} - Licitacion: {self.licitacion.rfq}"
    
class Proyecto_SE_SIA(models.Model):
    proyecto_se = models.ForeignKey(Proyecto_SE, on_delete=models.CASCADE)
    sia = models.ForeignKey(tblProyectos, on_delete=models.CASCADE)

    def __str__(self):
        return f"Proyecto SE: {self.proyecto_se.codigo} - SIA: {self.sia.CodProyecto}"
    
class Proyecto_SE_Secciones_MF(models.Model):
    proyecto_se = models.ForeignKey(Proyecto_SE, on_delete=models.CASCADE)
    ...
    def __str__(self):
        ...
        try:
            cod = self.proyecto_se.codigo
        except AttributeError:
            cod = "SIN ASIGNAR"
            
        return f"Proyecto SE: {cod} - Division: {self.division} - Seccion: {self.seccion}"
